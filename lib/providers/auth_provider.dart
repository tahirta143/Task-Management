...
//   String? get error => _error;
//   bool get isAuthenticated => _token != null;
//   String? get companyName => _companyName; // ADD THIS GETTER
//   // Check if user is admin
//   bool get isAdmin => _role == 'admin';
//
//   // Check if user is manager
...
class AuthProvider with ChangeNotifier {
  ...
  // Get company name only for managers - returns null for non-managers
  String? get displayCompanyName {
    if (_role?.toLowerCase() != 'manager') return null;
    return _companyName; // Return only company name
  }
  ...
  Future<void> _loadStoredData() async {
    try {
      ...
      print('Display Company (manager only): ${displayCompanyName}');
  ...
  Future<bool> login(String email, String password) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // Handle company field - extract name properly
        final companyField = data['user']['company'];
        _companyId = null;
        _companyName = null;

        if (companyField != null) {
          print('=== COMPANY FIELD DEBUG ===');
          print('Company field type: ${companyField.runtimeType}');
          print('Company field: $companyField');

          if (companyField is String) {
            _companyId = companyField;
            print('Company is a string (ID only): $companyField');
          } else if (companyField is Map<String, dynamic>) {
            // Extract company ID
            _companyId = companyField['_id'] ?? companyField['id'];

            // Extract company name - try different possible keys
            _companyName = companyField['name'] ??
                companyField['Name'] ??
            ...
            print('Company ID: $_companyId');
            print('Company Name found: $_companyName');

            // If no name found but we have ID, fetch company details
            if (_companyName == null && _companyId != null && _role?.toLowerCase() == 'manager') {
              print('Fetching company details for ID: $_companyId');
              await _fetchCompanyDetails(_companyId!);
            }
          }
          print('===========================');
        }
        ...
        print('Display Company Name: ${displayCompanyName}');
  ...
  Future<void> _fetchCompanyDetails(String companyId) async {
    try {
      print('Fetching company details for: $companyId');

      final response = await http.get(
        Uri.parse('https://task-management-backend-bn2k.vercel.app/api/companies/$companyId'),
        headers: {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        print('Company API Response: $data');
        ...
        if (data['success'] && data['data'] != null) {
          ...
          if (_companyName != null) {
            print('Successfully fetched company name: $_companyName');
            ...
            print('Company name not found in response');
          }
        }
      } else {
  ...
  Future<void> printStoredData() async {
    final prefs = await SharedPreferences.getInstance();
    print('=== CURRENT STORED DATA ===');
    print('Token: ${prefs.getString('token') != null ? "Exists" : "None"}');
    print('User ID: ${prefs.getString('userId')}');
    print('User Name: ${prefs.getString('userName')}');
    print('User Email: ${prefs.getString('userEmail')}');
    print('Role: ${prefs.getString('role')}');
  ...
  void setMockUser({String role = 'admin', String? companyName}) {
    ...
    if (role == 'manager') {
      _companyId = 'mock-company-id';
      _companyName = companyName ?? 'Mock Company';
    } else {
      _companyId = null;
      _companyName = null;
    }
