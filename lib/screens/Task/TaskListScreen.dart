...
import '../../models/task_model.dart';
import 'AddTaskScreen.dart';
import 'edittaskdialog.dart'; // Make sure this exists
...
class _TaskListScreenState extends State<TaskListScreen> {
  ...
  // screens/tasks/task_list_screen.dart - UPDATE DELETE METHOD
  Future<void> _deleteTask(BuildContext context, Task task) async {
    ...
    if (confirmDelete) {
      ...
      try {
        // Call delete API using TaskProvider
  ...
  Widget _buildTaskList(TaskProvider taskProvider) {
    ...
    // Create a new key based on task count to force ListView rebuild
    final listKey = ValueKey<int>(taskProvider.tasks.length);

    return RefreshIndicator(
      key: listKey, // Add key to RefreshIndicator
      onRefresh: () async {
        await taskProvider.fetchTasks();
      },
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          return TaskCard(
            ...
            // Important: Add unique key to each TaskCard
            task: task,
